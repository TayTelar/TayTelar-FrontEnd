...
const Login = () => {
    ...
    const sendOTP = async () => {
        ...
        try {
            const response = await axios.post('http://localhost:8085/api/otp/sendOtp', requestbody);
    ...
    const verifyOTP = async (otp: string) => {
        ...
        try {
            const response = await axios.post('http://localhost:8085/api/otp/verifyOtp', requestbody);
    ...
    const register = async (otp: string) => {
        ...
        if (otpValid) {
            ...
            try {
                const response = await axios.post('http://localhost:8085/api/user/register', requestbody);
    ...
    const login = async (otp: string) => {
        ...
        if (otpValid) {
            ...
            try {
                const response = await axios.post('http://localhost:8085/api/user/login', requestbody);
                ...
                let errorMessage = 'An error occurred login.';
                ...
                console.error('Error while login:', errorMessage);
                setErrorContent(errorMessage);
                handleErrorModal();
            }
        } else {
    ...
    const validationSchema = Yup.object().shape({
        ...
        otp: Yup.string().when('otpSent', (otpSent, schema) => {
            return otpSent ? schema.required('OTP is required') : schema.notRequired();
        }),

        firstName: Yup.string().when('isRegister', (isRegister, schema) => {
            return isRegister ? schema.required('First Name is required') : schema.notRequired();
        }),

        lastName: Yup.string().when('isRegister', (isRegister, schema) => {
            return isRegister ? schema.required('Last Name is required') : schema.notRequired();
        }),
                    ...
                    {!isRegister ? (
                        ...
                        <>
                                ...
                                <form onSubmit={formik.handleSubmit}>
                                    ...
                                    {otpSent && (
                                        <>
                                            ...
                                            />
                                            <FormHelperText>{formik.touched.otp && formik.errors.otp}</FormHelperText>
                                        </>
                                    )}
                                    {otpSent ? (
                                        <button className="login_btn" type="button" disabled={!formik.values.phoneNumber || !formik.values.otp} onClick={handleLogin}
                                        >
                                            Login
                                        </button>
                                    ) : (
                                        <button className="login_btn" type="button" disabled={!formik.values.phoneNumber} onClick={handleSendOtpClick}
                                        >
                                            Send OTP
                                        </button>
                                    )}
                                ...
                                <div className="social_login">
                                    ...
                                    >
                                        <GoogleLoginButton
                                            ...
                                            iconSize="18px"
                                        >
                                            <span style={{ color: '#000000', marginLeft: '15px' }}>Continue with Google</span>
                                        </GoogleLoginButton>
                                    </LoginSocialGoogle>
                                    ...
                                    >
                                        ...
                                        >
                                            <span style={{ color: '#000000', marginLeft: '15px' }}>Continue with Facebook</span>
                                        </FacebookLoginButton>
                                    </LoginSocialFacebook>
                                    ...
                                    <LoginSocialApple scope={'name email'}>
                                        <AppleLoginButton
                                            ...
                                            iconSize="18px"
                                        >
                                            <span style={{ color: '#000000', marginLeft: '15px' }}>Continue with Apple</span>
                                        </AppleLoginButton>
                                    </LoginSocialApple>
                                </div>
                            </div>
                            ...
                            <div className="register_button">
                                ...
                                <span onClick={() => { setIsRegister(true); formik.resetForm(); }} style={{ cursor: 'pointer', color: 'blue' }}>
                                    Register
                                </span>
                            </div>
                        </>
                        ...
                        <div className="login_container_section_register">
                            <form onSubmit={formik.handleSubmit}>
                                ...
                                {otpSent ? (
                                    <button className="login_btn" type="button" disabled={!formik.values.email || !formik.values.phoneNumber || !formik.values.firstName || !formik.values.lastName || !formik.values.otp} onClick={handleRegisterClick}>
                                        Register
                                    </button>
                                ) : (
                                    <button className="login_btn" type="button" disabled={!formik.values.email || !formik.values.phoneNumber || !formik.values.firstName || !formik.values.lastName} onClick={handleSendOtpClick}>
                                        Send OTP
                                    </button>
                                )}
                            ...
                            <p onClick={() => { setIsRegister(false); formik.resetForm(); }} style={{ cursor: 'pointer', color: 'blue' }}>
                                Login
                            </p>
                        </div>
                    )}
